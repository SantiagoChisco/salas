...
class Userauth {
	...
	//spl_autoload_register('clases/crud_model.php');
	...
	function trylogin($username, $password){
		$autenticacion = validarUsuario($username,$password);
		...
		if( $username != '' && $password != ''){
			...
			if($autenticacion){
				echo("Entro");
				$separar = explode(".", $username);
				$separar2 = explode("@", $separar[1]);
				$nombre = $separar[0];
				$apellido = $separar2[0];
				$espacio = " ";
				$nombreUsuario = $nombre . $espacio . $apellido;
			...
			$this->object->db->where('users.username', $nombreUsuario);
			...
			if($return == 1){
				...
				$sessdata['username'] = $nombreUsuario;
				...
				// Hash is <login_date><username><schoolcode><authlevel>
				$str = 'c0d31gn1t3r'.$timestamp.$nombreUsuario.$this->GetAuthLevel($row->user_id);
				...
				$sql = "INSERT INTO users (school_id,username,firstname,lastname,email,authlevel,enabled) VALUES (1,'$nombreUsuario','$nombre','$apellido','$username',2,1)";
				//$sql = "INSERT INTO usersgroups (groupid,userid) VALUES ('$groupid','$userid')";
				$query = $this->object->db->query($sql);
				
				
				//echo(crud_model::Add('users', 'X', $data));
